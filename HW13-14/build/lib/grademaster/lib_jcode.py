...
# I imported this library to finc the number of files in a directory
import glob
...
#TODO: take care of error for empty dirs
    ...
    return len(files) #int(subprocess.Popen(tmp_str,shell=True,stdout=subprocess.PIPE).stdout.read())
...
#TODO: take care of error for empty dirs
    ...
    # Again we'll be using the glob library
    files = glob.glob(dir)
    return len(files) #int(subprocess.Popen(tmp_str,shell=True,stdout=subprocess.PIPE).stdout.read())
...
def dsu_sort(list_, index, reverse=False):
    """(dsu_sort):
    This function sorts out the values of a list based on the index values
    """
# TODO: infoline
    # the variable name should not be the same as the data type which in here is the list: list -> list_
...
def dsu_sort2(list_, index, reverse=False):
    """(dsu_sort2):
        This function sorts only based on the primary element, not on secondary elements in case of equality.
    """
    # The same as above we should not use list as the name of a variable
    for i, e in enumerate(list_):
        list_[i] = e[index]
    if reverse:
        list_.sort(reverse=True)
